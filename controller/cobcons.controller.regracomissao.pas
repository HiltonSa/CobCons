...
begin
  ...
  try
    ...
    then TIntegerField(FQuery.FieldByName('parcela')).DisplayFormat := '000';
    if vLista.IndexOf('pclib') > -1
    then TFloatField(FQuery.FieldByName('pclib')).DisplayFormat := '000';
    if vLista.IndexOf('percentual') > -1
    then TFloatField(FQuery.FieldByName('percentual')).DisplayFormat := ',#0.0000';
