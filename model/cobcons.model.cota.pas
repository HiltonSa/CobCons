...
var
  vSql, vInfAdic: String;
begin
  FormatSettings.DecimalSeparator := '.';
  case Ptipo of
    1: begin
        vInfAdic := FEmail + Copy(FInfAdic2,51,59)+ FCelular;
        vSql := Format( FSQL.RetornaSqlAlterarEndereco,
                     [ QuotedStr(FTelefone),
                       QuotedStr(FEndereco),
                       QuotedStr(FBairro),
                       QuotedStr(FCidade),
...
function TModelCota.CELULAR: String;
begin
  FInfAdic2 := FQuery.FieldByName('INF_ADIC_2').AsString;
  Result := Copy(FInfAdic2,60,20);
end;

function TModelCota.EMAIL: String;
begin
  FInfAdic2 := FQuery.FieldByName('INF_ADIC_2').AsString;
  Result := Copy(FInfAdic2,1,50);
end;
...
function TModelCota.CELULAR(aValue: String): iModelCota;
begin
  Result := Self;
  FCelular := Avalue;
end;

function TModelCota.EMAIL(aValue: String): iModelCota;
begin
  Result := Self;
  FEmail := Avalue;
end;
