...
type
  ...
  TFrmPagamentoRnp = class(TForm)
    ...
    procedure gerarAsvrExecute(Sender: TObject);
...
uses cobcons.model.configura, cobcons.controller.factory,
  ...
  cobcons.view.exportarpagrnp, cobcons.view.gerarasvr,
  cobcons.view.rel.pagamentornp;
...
procedure TFrmPagamentoRnp.gerarAsvrExecute(Sender: TObject);
begin
  FrmGerarAsvr := TFrmGerarAsvr.Create(nil);
  try
     if FrmGerarAsvr.ShowModal = mrOK
     then begin
       MostraAviso('Fim da exportação!');
     end;
  finally
